...
get_ortho_clusters<-function(ingroup, outgroup, MSA=T, ALI=F){
  ...
  for (i in 1:length(seqs)) {
    ...
    sequence_names<-cbind.data.frame(
    name =word(names(sequences), 2, 3, fixed("_")),
    ...
    write.dna(
      as.DNAbin(sequences),
      file = paste0(ingroup, "_", "Cluster" , i, ".fasta"), format = "fasta"
    )
  ...
  file.remove(list.files(".",pattern = "cluster|sequences", include.dirs = F, full.names = T, recursive = T))
